                ...
                <View style={{ flex: 1, minWidth: 0, justifyContent: 'center' }}>
                  ...
                  <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 2, justifyContent: 'flex-end' }}>
                    ...
                    <TouchableOpacity
                      ...
                      onPress={() => {
                        try {
                          // Store conversation details in AsyncStorage for the chat screen to use
                          AsyncStorage.setItem('currentChatFriend', JSON.stringify({
                            id: friend._id,
                            name: friend.name
                          }));
                          
                          // Navigate directly to the standalone chat screen (outside tabs)
                          router.navigate('/screens/ChatScreen' as any);
                          
                          // Try to create a conversation in the background (non-blocking)
                          AsyncStorage.getItem('token').then((storedToken) => {
                            axios.post(
                              ...
                              { participants: [friend._id] },
                              {
                                ...
                                // Add timeout to avoid long-pending requests
                                timeout: 5000
                              }
                            ).then((response: { data: { _id: string } }) => {
                              if (response.data && response.data._id) {
                                // Store conversation ID for the chat screen to use later
                                AsyncStorage.setItem('currentConversationId', response.data._id);
                              }
                            }).catch(() => {
                              // Silently fail - no console error logs
                              // This is background processing, user doesn't need to know
                            });
                          }).catch(() => {
